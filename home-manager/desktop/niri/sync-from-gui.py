...
Run this after configuring noctalia in the GUI to sync changes to the Nix config.
"""
...
def main():
    ...
    nix_file = script_dir / "noctalia.nix"
    ...
    backup_file = backup_dir / f"noctalia.nix.{timestamp}"
...
{{
  # Import noctalia home module if input is available
  imports = lib.optionals (inputs ? noctalia) [
  ...
  config = lib.mkIf (inputs ? noctalia) {{
    ...
    # Noctalia settings template
    home.file.".config/noctalia/settings.json.template" = {{
      text = builtins.toJSON {settings};
    }};

    # Initialize settings.json from template if it doesn't exist
    ...
    home.activation.noctaliaWarning = lib.hm.dag.entryAfter [ "noctaliaSettingsInit" ] ''
      ...
      $DRY_RUN_CMD echo "   cd home-manager/desktop/niri && ./sync-from-gui.py"
      $DRY_RUN_CMD echo ""
...
'''
    ...
    print("  1. Review: git diff home-manager/desktop/niri/noctalia.nix")
    print("  2. Rebuild: sudo nixos-rebuild switch --flake .#penglaptop")
